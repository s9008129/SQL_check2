...
# Evidence can support either a performance-review principle or an Oracle
# correctness/behavior boundary. A supported rewrite such as same-column
# OR→IN may therefore carry Oracle documentation even when the evidence does
# *not* claim a speedup. The caveat text is the authority for that distinction.
...
def _default_applicability(pattern_id: str) -> str:
    return {
        ...
        "TRUNC_EQ_TO_RANGE": (
            "這支 SQL 的 TRUNC() 是條件式直接比較的一側；是否改成日期範圍仍需先確認欄位型態與時間條件。"
        ),
        "NVL_EQ_TO_OR_IS_NULL": (
            "這支 SQL 的 NVL()/COALESCE() 是條件式直接比較的一側；是否拆成明確條件仍需先確認欄位型態與 NULL 規則。"
        ),
        "TO_CHAR_CONDITION_PREDICATE": (
            "這支 SQL 的 TO_CHAR() 是條件式直接比較的一側；可評估是否能直接以原欄位型態比對。"
        ),
        "UPPER_CASE_FOLD_REMOVAL": (
            "這支 SQL 的 UPPER()/LOWER() 是條件式直接比較的一側；調整前需先確認資料的大小寫規則。"
        ),
        "DISTINCT_REMOVAL": (
            "這支 SQL 使用 DISTINCT 去除重複資料；先確認去重是否真的是業務需求，再決定是否需要調整。"
        ),
...
def build_performance_evidence(
    ...
    Family signals never receive evidence. Evidence may explain a performance
    principle *or* an Oracle correctness boundary; its caveat must say when it
    is not evidence of a speedup. SUBSTR is refined using the deterministic
    canonical rewrite: prefix LIKE and leading-wildcard LIKE deliberately
    receive different access-path wording.
    """
