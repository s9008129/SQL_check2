...
| `LATEST_ROW_CORRELATED_MAX` | Replacing repeated correlated MAX lookups with ROW_NUMBER()=1 or one joined latest-row source can drop tied rows when date/time is not unique; SQL text alone cannot prove the tie-break business rule |
| `REPEATED_SCALAR_AGGREGATE` | Consolidating repeated scalar aggregates requires proving their filters, correlation, NULL handling and grouping are compatible; sharing a source table alone is not enough |
| `COMPOSITE_KEY_EXPRESSION_JOIN` | Splitting SUBSTR/concatenated join keys needs verified segment widths, CHAR/VARCHAR2 padding, NULL behavior and the real business key; SQLCheck cannot infer these from usage |
| `REPEATED_SOURCE_UNION_BRANCH` | Same-source UNION/UNION ALL branches may be candidates for CASE/conditional aggregation, but branch overlap, deduplication and row-shape semantics can change the result |
